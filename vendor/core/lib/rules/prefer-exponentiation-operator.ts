...
/**
 * @file Rule to disallow Math.pow in favor of the ** operator
...
const { ReferenceTracker } = dependency1;
 ...
 */
function doesBaseNeedParens(base: Node) {
    return (
 ...
 * @returns `true` if the node needs to be parenthesised.
 */
function doesExponentNeedParens(exponent: Node) {
 ...
 */
function doesExponentiationExpressionNeedParens(node: Node, sourceCode: SourceCode) {
 ...
 */
function parenthesizeIfShould(text: string, shouldParenthesize: boolean) {
...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
         ...
         */
        function report(node: Node<'CallExpression'>) {
            context.report({
                node,
                ...
                fix(fixer: Fixer) {
                    ...
                    const shouldParenthesizeAll = doesExponentiationExpressionNeedParens(
                        node,
                        sourceCode,
                    );
                    ...
                    const exponentReplacement = parenthesizeIfShould(
                        exponentText,
                        shouldParenthesizeExponent,
                    );
                    const replacement = parenthesizeIfShould(
