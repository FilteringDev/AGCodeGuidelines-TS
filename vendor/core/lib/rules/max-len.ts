/**
 * @file Rule to check for max length on a line.
...
> = {
    ...
    create(context) {
        ...
        if (typeof context.options[0] === 'number') {
            [options.code] = context.options;
        }

        // …and tabWidth can be optionally specified directly as integers.
        if (typeof context.options[1] === 'number') {
            [, options.tabWidth] = context.options;
        }
         ...
         */
        function isTrailingComment(line: string, lineNumber: number, comment: Comment | Node) {
            return (
                comment
         ...
         */
        function isFullLineComment(line: string, lineNumber: number, comment: Comment | Node) {
         ...
         */
        function isJSXEmptyExpressionInSingleLineContainer(node: Node) {
            if (
                !node
                || !node.parent
         ...
         */
        function stripTrailingComment(line: string, comment: Comment | Node) {
         ...
         */
        function ensureArrayAndPush(
            object: Record<number, Token[]>,
            key: number,
            value: Token,
        ) {
        ...
        function getAllTemplateLiterals() {
            return sourceCode.ast.tokens.filter((token: Token) => token.type === 'Template');
        }
        ...
        function getAllRegExpLiterals() {
            return sourceCode.ast.tokens.filter(
                (token: Token) => token.type === 'RegularExpression',
            );
        }
         ...
         */
        function groupArrayByLineNumber(arr: Token[]) {
            const obj: Record<number, Token[]> = {};

            for (let i = 0; i < arr.length; i += 1) {
                const node = arr[i];

                for (let j = node!.loc.start.line; j <= node!.loc.end.line; j += 1) {
         ...
         */
        function checkProgramForMaxLength(node: Node<'Program'>) {
            // split (honors line-ending)
            const { lines } = sourceCode;

            // list of comments to ignore
            const comments: (Comment | Node)[] = ignoreComments || maxCommentLength || ignoreTrailingComments
                ? getAllComments()
                : [];
