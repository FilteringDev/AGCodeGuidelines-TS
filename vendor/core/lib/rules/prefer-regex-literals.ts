...
/**
 * @file Rule to disallow use of the `RegExp` constructor in favor of regular expression literals
 * @author Milos Djermanovic
 */
import * as dependency2 from '@eslint-community/regexpp';
import dependency1 from '../../compat/eslint-utils';
import dependency0 from './utils/ast-utils';
import dependency3 from './utils/regular-expressions';
...
const { ReferenceTracker, findVariable } = dependency1;
const { RegExpValidator, visitRegExpAST, RegExpParser } = dependency2;
const { canTokensBeAdjacent } = dependency0;
const { REGEXPP_LATEST_ECMA_VERSION } = dependency3;
 ...
 */
function isRegexLiteral(node: Node) {
...
const rule: LegacyRule<[{ disallowRedundantWrapping?: boolean }?]> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
         ...
         */
        function isGlobalReference(node: Node<'Identifier'>) {
         ...
         */
        function isStringRawTaggedStaticTemplateLiteral(
            node: Node,
        ): node is Node<'TaggedTemplateExpression'> {
            return (
         ...
         */
        function isStaticString(node: Node) {
            return (
         ...
         */
        function hasOnlyStaticStringArguments(node: Node<'CallExpression' | 'NewExpression'>) {
        ...
        /**
         * Determines whether the arguments of the given node indicate that a regex literal is unnecessarily
         * wrapped.
         * @param node Node to check.
         * @returns True if the node already contains a regex literal argument.
         */
        function isUnnecessarilyWrappedRegexLiteral(
            node: Node<'CallExpression' | 'NewExpression'>,
        ): node is Node<'CallExpression' | 'NewExpression'> & {
            arguments: [
                Node<'Literal'> & { regex: { pattern: string; flags: string } },
                ...Node[],
            ];
        } {
            const args = node.arguments;
         ...
         */
        function getRegexppEcmaVersion(ecmaVersion: number) {
            if (ecmaVersion <= 5) {
                return 5;
            }
        ...
        ) as NonNullable<ConstructorParameters<typeof RegExpValidator>[0]>['ecmaVersion'];
         ...
         */
        function resolveEscapes(character: string) {
            switch (character) {
                case '\n':
                case '\\\n':
                    return '\\n';
         ...
         */
        function isValidRegexForEcmaVersion(pattern: string, flags: string | undefined) {
         ...
         */
        function areFlagsEqual(flagsA: string, flagsB: string) {
         ...
         */
        function mergeRegexFlags(flagsA: string, flagsB: string) {
         ...
         */
        function canFixTo(node: Node, pattern: string, flags: string) {
         ...
         */
        function getSafeOutput(node: Node, newRegExpValue: string) {
            const tokenBefore = sourceCode.getTokenBefore(node);
                    ...
                    ({ node: refNode }) => {
                        if (
                            ...
                            if (refNode.arguments.length === 2) {
                                ...
                                const literalFlags = sourceCode
                                    .getText(regexNode)
                                    .slice(sourceCode.getText(regexNode).lastIndexOf('/') + 1);
