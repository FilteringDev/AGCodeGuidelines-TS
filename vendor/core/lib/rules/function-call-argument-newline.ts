/**
 * @file Rule to enforce line breaks between arguments of a function call
...
import type {
    Node, Fixer, LegacyRule, Token,
} from '../../../types';

const rule: LegacyRule<[('always' | 'never' | 'consistent')?]> = {
    meta: {
        deprecated: true,
        replacedBy: [],
        type: 'layout',
    ...
    create(context) {
        ...
        const checkers = {
            unexpected: {
                ...
                check: (before: Token, after: Token) => before.loc.end.line !== after.loc.start.line,
                createFix: (token: Token, tokenBefore: Token) => (fixer: Fixer) => fixer.replaceTextRange([tokenBefore.range[1], token.range[0]], ' '),
            },
            missing: {
         ...
         * @param checker.messageId The message id value.
         * @param checker.check The check value.
         */
        function checkArguments(
            node: Node<'CallExpression' | 'NewExpression'>,
            checker: typeof checkers.unexpected,
        ) {
