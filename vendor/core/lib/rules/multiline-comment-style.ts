/**
 * @file enforce a particular style for multiline comments
 * @author Teddy Katz
 */
...
const rule: LegacyRule<
    [('starred-block' | 'bare-block')?] | ['separate-lines'?, { checkJSDoc?: boolean }?]
> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
         ...
         */
        function isStarredCommentLine(line: string) {
            return /^\s*\*/u.test(line);
        }

        /**
         * Checks if a comment group is in starred-block form.
         * @param commentGroup A group of comments, containing either multiple line comments or a single block
         * comment.
         * @param commentGroup."0" The "0" value.
         * @returns Whether or not the comment group is in starred block form.
         */
        function isStarredBlockComment([firstComment]: Token[]) {
            ...
            return (
                lines.length > 0
                && lines.every((line, i) => (i === 0 || i === lines.length - 1 ? /^\s*$/u : /^\s*\*/u).test(line))
            );
        }
         ...
         */
        function isJSDocComment([firstComment]: Token[]) {
            if (firstComment!.type !== 'Block') {
                return false;
            }
         ...
         */
        function processSeparateLineComments(commentGroup: Token[]) {
         ...
         * @returns An array of the processed lines.
         */
        function processStarredBlockComment(comment: Token) {
         ...
         * @returns An array of the processed lines.
         */
        function processBareBlockComment(comment: Token) {
            const lines = comment.value
                .split(astUtils.LINEBREAK_MATCHER)
                .map((line) => line.replace(/^\s*$/u, ''));
            ...
            /**
             * Calculate the offset of the least indented line and use that as the basis for offsetting all the
             * lines.
             * The first line should not be checked because it is inline with the opening block comment delimiter.
             */
            Array.from(lines.entries()).forEach(([i, line]) => {
         ...
         */
        function getCommentLines(commentGroup: Token[]) {
         ...
         */
        function getInitialOffset(comment: Token) {
         ...
         */
        function convertToStarredBlock(firstComment: Token, commentLinesList: string[]) {
         ...
         */
        function convertToSeparateLines(firstComment: Token, commentLinesList: string[]) {
         ...
         */
        function convertToBlock(firstComment: Token, commentLinesList: string[]) {
         ...
         * block comment or multiple line comments.
          @returns */
        const commentGroupCheckers: Record<
            'starred-block' | 'bare-block' | 'separate-lines',
            (commentGroup: Token[]) => void
        > = {
            'starred-block': function onStarredBlock(commentGroup) {
                const [firstComment] = commentGroup;
                ...
                if (commentGroup.length > 1) {
                    ...
                    if (!/^\*?\s*$/u.test(lines[0]!)) {
                        ...
                        context.report({
                            ...
                            fix: (fixer: Fixer) => fixer.insertTextAfterRange(
                                [start, start + 2],
                                `\n${expectedLinePrefix}`,
                            ),
                        });
                        ...
                        if (!lineText!.startsWith(expectedLinePrefix)) {
                            context.report({
                                ...
                                fix(fixer: Fixer) {
                                    ...
                                    const indexedLines = Array.from(lines.entries());
                                    for (
                                        let alignmentIndex = 0;
                                        alignmentIndex < indexedLines.length;
                                        alignmentIndex += 1
                                    ) {
                                        const [idx, line] = indexedLines[alignmentIndex]!;
                                        if (/\S+/u.test(line)) {
                                            const lineTextToAlignWith = sourceCode.lines[
                                                firstComment!.loc.start.line - 1 + idx
                                            ];
            ...
            'separate-lines': function onSeparateLines(commentGroup) {
                const [firstComment] = commentGroup;
            ...
            'bare-block': function onBareBlock(commentGroup) {
        ...
        return {
            Program() {
                return sourceCode
                    .getAllComments()
                    .filter((comment: Token) => comment.type !== 'Shebang')
                    .filter(
                        (comment: Token) => !astUtils.COMMENTS_IGNORE_PATTERN.test(comment.value),
                    )
                    ...
                    .reduce<Token[][]>((commentGroups, comment: Token, index, commentList) => {
                        const tokenBefore = sourceCode.getTokenBefore(comment, {
                            includeComments: true,
                        });
