...
import type {
    Scope, Fixer, LegacyRule, Node, Variable,
} from '../../../types';
 ...
 */
function checkMetaProperty(node: Node<'MetaProperty'>, metaName: string, propertyName: string) {
 ...
 * @throws (Unreachable.)
 * @returns *   {boolean} retv.isCallback - `true` if the node is a callback.
 *   {boolean} retv.isLexicalThis - `true` if the node is with `.bind(this)`.
 */
function getCallbackInfo(node: Node<'FunctionExpression'>) {
 ...
 */
function hasDuplicateParams(paramsList: Node[]) {
    return (
...
const rule: LegacyRule<[{ allowNamedFunctions?: boolean; allowUnboundThis?: boolean }?]> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
         ...
         */
        let stack: { this: boolean; super: boolean; meta: boolean }[] = [];
                    ...
                    context.report({
                        ...
                        * fix(fixer: Fixer) {
                            if (
                                 ...
                                 * If the callback function has duplicates in its list of parameters (possible in
                                 * sloppy mode),
                                 * don't replace it with an arrow function, because this is a SyntaxError with arrow
                                 * functions.
                                 */
                                return;
                            }
                            ...
                            if (callbackInfo.isLexicalThis) {
                                ...
                                /**
                                 * If `.bind(this)` exists but the parent is not `.bind(this)`, don't remove it
                                 * automatically.
                             ...
                             * the arrow function needs to be parenthesized, because `foo || () => {}` is invalid
                             * syntax even
