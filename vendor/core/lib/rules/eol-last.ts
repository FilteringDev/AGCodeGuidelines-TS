/**
 * @file Require or disallow newline at the end of files
...
const rule: LegacyRule<[('always' | 'never' | 'unix' | 'windows')?]> = {
    meta: {
        deprecated: true,
        replacedBy: [],
        type: 'layout',
    ...
    create(context) {
        ...
        return {
            Program: function checkBadEOF(node: Node<'Program'>) {
                ...
                if (mode === 'windows') {
                    // `"windows"` should behave exactly as `"always"`, but append CRLF in the fixer for backwards
                    // compatibility
                    mode = 'always';
                    appendCRLF = true;
                }
