/**
 * @file Rule to enforce spacing around embedded expressions of template strings
        ...
        function checkSpacingBefore(token: Token) {
            ...
            if (always && !hasSpace) {
                context.report({
                    ...
                    fix: (fixer: Fixer) => fixer.insertTextBefore(token, ' '),
                });
            }
            ...
            if (!always && hasSpace) {
                context.report({
                    ...
                    fix: (fixer: Fixer) => fixer.removeRange([prevToken!.range[1], token.range[0]]),
                });
            }
        }
        ...
        function checkSpacingAfter(token: Token) {
            ...
            if (always && !hasSpace) {
                context.report({
                    ...
                    fix: (fixer: Fixer) => fixer.insertTextAfter(token, ' '),
                });
            }
            ...
            if (!always && hasSpace) {
                context.report({
                    ...
                    fix: (fixer: Fixer) => fixer.removeRange([token.range[1], nextToken!.range[0]]),
                });
            }
        }

        return {
            TemplateElement(node: Node<'TemplateElement'>) {
