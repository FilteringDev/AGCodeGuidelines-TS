/**
 * @file Rule to flag the use of redundant constructors in classes.
 ...
 */
function isSingleSuperCall(
    body: Node[],
): body is [Node<'ExpressionStatement'> & { expression: Node<'CallExpression'> }] {
    return (
        body.length === 1
...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
         ...
         */
        function checkForConstructor(node: Node<'MethodDefinition'>) {
            ...
            // Constructors are methods in a class body.
            const { superClass } = node.parent.parent as Node<
