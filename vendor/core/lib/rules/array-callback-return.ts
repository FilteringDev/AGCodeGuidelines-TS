...
/**
 * @file Rule to enforce return statements in callbacks of array's methods
 ...
 */
function isExpressionVoid(node: Node) {
...
const rule: LegacyRule<
    [{ allowImplicit?: boolean; checkForEach?: boolean; allowVoid?: boolean }?]
> = {
    meta: {
        type: 'problem',
            ...
            onCodePathStart(codePath: CodePath, node: Node) {
                ...
                if (TARGET_NODE_TYPE.test(node.type)) {
                    methodName = getArrayMethodName(
                        node as Node<'FunctionExpression' | 'ArrowFunctionExpression'>,
                    );
                }
