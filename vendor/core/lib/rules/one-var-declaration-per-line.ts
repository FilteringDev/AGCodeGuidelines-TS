/**
 * @file Rule to check multiple var declarations per line
 * @author Alberto Rodríguez
 * @deprecated in ESLint v8.53.0
 */
import type { Fixer, LegacyRule, Node } from '../../../types';

//------------------------------------------------------------------------------
// Rule Definition
//------------------------------------------------------------------------------

const rule: LegacyRule<[('always' | 'initializations')?]> = {
    meta: {
        deprecated: true,
        replacedBy: [],
        type: 'suggestion',

        docs: {
            description: 'Require or disallow newlines around variable declarations',
            recommended: false,
            url: 'https://eslint.org/docs/latest/rules/one-var-declaration-per-line',
        },

        schema: [
            {
                enum: ['always', 'initializations'],
            },
        ],

        fixable: 'whitespace',

        messages: {
            expectVarOnNewline: 'Expected variable declaration to be on a new line.',
        },
    },

    create(context) {
        const always = context.options[0] === 'always';

        //--------------------------------------------------------------------------
        // Helpers
        //--------------------------------------------------------------------------

        /**
         * Determine if provided keyword is a variant of for specifiers
         * @param keyword keyword to test
         * @returns True if `keyword` is a variant of for specifier
         */
        function isForTypeSpecifier(keyword: string) {
            return (
                keyword === 'ForStatement'
                || keyword === 'ForInStatement'
                || keyword === 'ForOfStatement'
            );
        }

        /**
         * Checks newlines around variable declarations.
         * @param node `VariableDeclaration` node to test
         */
        function checkForNewLine(node: Node<'VariableDeclaration'>) {
            if (isForTypeSpecifier(node.parent.type)) {
                return;
            }

            const { declarations } = node;
            let prev: Node<'VariableDeclarator'> | null | undefined;

            declarations.forEach((current) => {
                if (prev && prev.loc.end.line === current.loc.start.line) {
                    if (always || prev.init || current.init) {
                        context.report({
                            node,
                            messageId: 'expectVarOnNewline',
                            loc: current.loc,
                            fix: (fixer: Fixer) => fixer.insertTextBefore(current, '\n'),
                        });
                    }
                }
                prev = current;
            });
        }

        //--------------------------------------------------------------------------
        // Public
        //--------------------------------------------------------------------------

        return {
            VariableDeclaration: checkForNewLine,
        };
    },
};

export default rule;
