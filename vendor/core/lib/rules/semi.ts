/**
 * @file Rule to flag missing semicolons.
...
> = {
    ...
    create(context) {
         ...
         */
        function report(node: Node, missing?: boolean) {
         ...
         *      removed.
         */
        function maybeClassFieldAsiHazard(node: Node) {
         ...
         */
        function isOnSameLineWithNextToken(node: Node) {
         ...
         */
        function maybeAsiHazardAfter(node: Node) {
            const t = node.type;
         ...
         */
        function canRemoveSemicolon(node: Node) {
         ...
         */
        function isLastInOneLinerBlock(node: Node) {
            const { parent } = node;
            const nextToken = sourceCode.getTokenAfter(node);
         ...
         */
        function isLastInOneLinerClassBody(node: Node) {
            const { parent } = node;
            const nextToken = sourceCode.getTokenAfter(node);
         ...
         * @param node The node to check.
         */
        function checkForSemicolonForVariableDeclaration(node: Node) {
            const { parent } = node;
