/**
 * @file Rule to forbid or enforce dangling commas.
...
type Mode = 'always-multiline' | 'always' | 'never' | 'only-multiline' | 'ignore';
type CommaOptions = Record<'arrays' | 'objects' | 'imports' | 'exports' | 'functions', Mode>;

const DEFAULT_OPTIONS: CommaOptions = Object.freeze({
    arrays: 'never',
    objects: 'never',
    imports: 'never',
    exports: 'never',
 ...
 */
function isTrailingCommaAllowed(lastItem: Node) {
    return !new Set(['RestElement', 'RestProperty', 'ExperimentalRestProperty']).has(
        lastItem.type,
    );
}
...
function normalizeOptions(
    optionValue: Mode | Partial<CommaOptions> | undefined,
    ecmaVersion: number,
): CommaOptions {
...
> = {
    ...
    create(context) {
         ...
         */
        function getLastItem(node: Node) {
            /**
             * Returns the last element of an array
             * @param array The input array
             * @returns The last element
             */
            function last(array: readonly (Node | null)[]): Node | null | undefined {
         ...
         */
        function getTrailingToken(node: Node, lastItem: Node) {
            switch (node.type) {
                case 'ObjectExpression':
                case 'ArrayExpression':
                case 'CallExpression':
         ...
         */
        function isMultiline(node: Node) {
         ...
         */
        function forbidTrailingComma(node: Node) {
            const lastItem = getLastItem(node);
         ...
         *   ImportDeclaration, and ExportNamedDeclaration.
         */
        function forceTrailingComma(node: Node) {
            const lastItem = getLastItem(node);
         ...
         *   ImportDeclaration, and ExportNamedDeclaration.
         */
        function forceTrailingCommaIfMultiline(node: Node) {
            if (isMultiline(node)) {
         ...
         *   ImportDeclaration, and ExportNamedDeclaration.
         */
        function allowTrailingCommaIfMultiline(node: Node) {
            if (!isMultiline(node)) {
                forbidTrailingComma(node);
            }
        }
