/**
 * @file Rule to require or disallow yoda comparisons
 ...
 */
function isRangeTestOperator(operator: string) {
...
function isNegativeNumericLiteral(
    node: Node,
): node is Node<'UnaryExpression'> & {
    argument: Node<'Literal'> & { value: number | bigint };
} {
    return (
 ...
 */
function looksLikeLiteral(node: Node) {
 ...
 *  4. Otherwise `null`.
 */
function getNormalizedLiteral(node: Node) {
...
const rule: LegacyRule<
    [('always' | 'never')?, { exceptRange?: boolean; onlyEquality?: boolean }?]
> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
         ...
         */
        function isRangeTest(node: Node) {
            if (
         ...
         */
        function getFlippedString(node: Node<'BinaryExpression'>) {
            ...
            BinaryExpression(node: Node<'BinaryExpression'>) {
                ...
                if (
                    ...
                    context.report({
                        ...
                        fix: (fixer: Fixer) => fixer.replaceText(node, getFlippedString(node)),
                    });
                }
            },
        };
