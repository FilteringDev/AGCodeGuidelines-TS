/**
 * @file Rule to replace assignment expressions with operator assignment
 ...
 *     shorthand form.
 */
function isCommutativeOperatorWithShorthand(operator: string) {
 ...
 *     a shorthand form.
 */
function isNonCommutativeOperatorWithShorthand(operator: string) {
    return ['+', '-', '/', '%', '<<', '>>', '>>>', '**'].includes(operator);
}
...
const rule: LegacyRule<[('always' | 'never')?]> = {
    ...
    create(context) {
         ...
         */
        function getOperatorToken(node: Node<'AssignmentExpression' | 'BinaryExpression'>) {
         ...
         */
        function verify(node: Node<'AssignmentExpression'>) {
         ...
         * @param node An AssignmentExpression node.
         */
        function prohibit(node: Node<'AssignmentExpression'>) {
            if (
                node.operator !== '='
                ...
                context.report({
                    ...
                    fix(fixer: Fixer) {
                        if (canBeFixed(node.left)) {
                            ...
                            // If this change would modify precedence (e.g. `foo *= bar + 1` => `foo = foo * (bar +
                            // 1)`), parenthesize the right side.
                            if (
