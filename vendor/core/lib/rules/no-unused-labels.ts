/**
 * @file Rule to disallow unused labels.
...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
         ...
         */
        function enterLabeledScope(node: Node<'LabeledStatement'>) {
            scopeInfo = {
         ...
         */
        function isFixable(node: Node<'LabeledStatement'>) {
            /**
         ...
         */
        function exitLabeledScope(node: Node<'LabeledStatement'>) {
            if (!scopeInfo!.used) {
                context.report({
                    node: node.label,
                    messageId: 'unused',
                    data: node.label,
                    fix: isFixable(node)
                        ? (fixer: Fixer) => fixer.removeRange([node.range[0], node.body.range[0]])
                        : null,
                });
            }
         ...
         *      ContinueStatement.
         */
        function markAsUsed(node: Node<'BreakStatement' | 'ContinueStatement'>) {
            if (!node.label) {
                return;
            }
