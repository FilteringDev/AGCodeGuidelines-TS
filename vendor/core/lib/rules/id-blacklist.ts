 ...
 */
function isRenamedImport(node: Node<'Identifier'>) {
    const { parent } = node;
 ...
 */
function isRenamedInDestructuring(node: Node<'Identifier'>) {
    const { parent } = node;
 ...
 */
function isShorthandPropertyDefinition(node: Node<'Identifier'>) {
    const { parent } = node;
...
const rule: LegacyRule<string[]> = {
    ...
    create(context) {
         ...
         */
        function shouldCheck(node: Node<'Identifier'>) {
            const { parent } = node;
         ...
         */
        function report(node: Node<'Identifier'>) {
            /**
