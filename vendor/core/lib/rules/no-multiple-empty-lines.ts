/**
 * @file Disallows multiple blank lines.
...
const rule: LegacyRule<[{ max: number; maxEOF?: number; maxBOF?: number }?]> = {
    meta: {
        deprecated: true,
        replacedBy: [],
        type: 'layout',
    ...
    create(context) {
        ...
        if (context.options.length) {
            ...
            maxEOF = typeof context!.options[0]!.maxEOF !== 'undefined'
                ? context!.options[0]!.maxEOF
                : max;
            maxBOF = typeof context!.options[0]!.maxBOF !== 'undefined'
        ...
        const allLines = sourceCode.lines[sourceCode.lines.length - 1] === ''
            ? sourceCode.lines.slice(0, -1)
            : sourceCode.lines;
        const templateLiteralLines: Set<number> = new Set();
        ...
        return {
            TemplateLiteral(node: Node<'TemplateLiteral'>) {
                node.quasis.forEach((literalPart) => {
                    // Empty lines have a semantic meaning if they're inside template literals. Don't count these as
                    // empty lines.
                    for (
            ...
            'Program:exit': function onProgramExit(node: Node<'Program'>) {
                return (
                    allLines
                        ...
                        // Given two line numbers of non-empty lines, report the lines between if the difference is
                        // too large.
