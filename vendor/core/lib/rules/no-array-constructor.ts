...
/**
 * @file Disallow construction of dense arrays using the Array constructor
...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
         ...
         */
        function getArgumentsText(node: Node<'CallExpression' | 'NewExpression'>) {
            const lastToken: Node | Token | null = sourceCode.getLastToken(node);

            if (!isClosingParenToken(lastToken)) {
                return '';
            }

            let firstToken: Node | Token | null = node.callee;
