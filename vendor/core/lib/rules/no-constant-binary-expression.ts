/**
 * @file Rule to flag constant comparisons and logical expressions that always/never short circuit
 * @author Jordan Eldredge <https://jordaneldredge.com>
 */
import * as dependency0 from 'globals';
import dependency1 from './utils/ast-utils';
import type { LegacyRule, Node, Scope } from '../../../types';

const globals = dependency0;
const {
 ...
 */
function isNullOrUndefined(scope: Scope, node: Node) {
    return (
        isNullLiteral(node)
 ...
 */
function hasConstantNullishness(scope: Scope, node: Node, nonNullish: boolean): boolean {
 ...
 */
function isStaticBoolean(scope: Scope, node: Node) {
    switch (node.type) {
        case 'Literal':
 ...
 */
function hasConstantLooseBooleanComparison(scope: Scope, node: Node) {
    switch (node.type) {
        ...
        case 'ArrayExpression': {
            ...
            const nonSpreadElements = node.elements.filter((e) => e !== null && e.type !== 'SpreadElement');
 ...
 */
function hasConstantStrictBooleanComparison(scope: Scope, node: Node) {
    switch (node.type) {
 ...
 */
function isAlwaysNew(scope: Scope, node: Node): boolean {
    switch (node.type) {
        case 'ObjectExpression':
        case 'ArrayExpression':
        case 'ArrowFunctionExpression':
 ...
 */
function findBinaryExpressionConstantOperand(scope: Scope, a: Node, b: Node, operator: string) {
