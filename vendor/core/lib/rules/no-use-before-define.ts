/**
 * @file Rule to flag use of variables before they are defined
 * @author Ilya Volodin
 */
 ...
 */
function isInClassStaticInitializerRange(node: Node<'ClassBody'>, location: number) {
    return node.body.some(
        (classMember) => (classMember.type === 'StaticBlock' && isInRange(classMember, location))
 ...
 */
function isClassStaticInitializerScope(scope: Scope) {
    ...
    if (scope.type === 'class-field-initializer') {
        ...
        return propertyDefinition.type === 'PropertyDefinition' && propertyDefinition.static;
    }
 ...
 * Static class field initializers and class static blocks are automatically run during the class definition
 * evaluation,
 ...
 */
function isFromSeparateExecutionContext(reference: Reference) {
 ...
 */
function isEvaluatedDuringInitialization(reference: Reference) {
...
> = {
    ...
    create(context) {
         ...
         */
        function shouldCheck(reference: Reference) {
            // Type annotations do not execute and cannot observe a temporal dead zone.
            if (reference.init || (reference.isTypeReference && !reference.isValueReference)) {
                return false;
            }
         ...
         */
        function checkReferencesInScope(scope: Scope) {
            scope.references.filter(shouldCheck).forEach((reference: Reference) => {
                const variable = reference.resolved;
