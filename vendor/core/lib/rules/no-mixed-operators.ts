/**
 * @file Rule to disallow mixed binary operators.
 ...
 * @param options.groups The groups value.
 * @param options.allowSamePrecedence The allow same precedence value.
 * @returns Normalized option object.
 */
function normalizeOptions(
    options: { groups?: string[][]; allowSamePrecedence?: boolean } = {},
) {
 ...
 */
function includesBothInAGroup(groups: Array<string[]>, left: string, right: string) {
...
function getChildNode(
    node: Node<'BinaryExpression' | 'LogicalExpression' | 'ConditionalExpression'>,
) {
...
> = {
    ...
    create(context) {
         ...
         */
        function shouldIgnore(node: Node<'BinaryExpression' | 'LogicalExpression'>) {
            const a = node;
            const b = node.parent;
         ...
         */
        function isMixedWithParent(node: Node<'BinaryExpression' | 'LogicalExpression'>) {
            return (
         ...
         *      them, too.
         */
        function reportBothOperators(node: Node<'BinaryExpression' | 'LogicalExpression'>) {
            // The listener only reports an operator nested in another operator.
         ...
         */
        function check(node: Node<'BinaryExpression' | 'LogicalExpression'>) {
            if (
