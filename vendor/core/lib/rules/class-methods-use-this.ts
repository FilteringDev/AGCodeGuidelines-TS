/**
 * @file Rule to enforce that all class methods use 'this'.
...
const rule: LegacyRule<[{ exceptMethods?: string[]; enforceForClassFields?: boolean }?]> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
        ...
        function isInstanceMethod(
            node: Node,
        ): node is Node<'MethodDefinition' | 'PropertyDefinition'> {
            switch (node.type) {
         ...
         */
        function isIncludedInstanceMethod(node: Node) {
