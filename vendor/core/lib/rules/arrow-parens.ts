/**
 * @file Rule to require parens in arrow function arguments.
 * @author Jxck
 ...
 */
function hasBlockBody(node: Node<'ArrowFunctionExpression'>) {
...
const rule: LegacyRule<[('always' | 'as-needed')?, { requireForBlockBody?: boolean }?]> = {
    meta: {
        deprecated: true,
        replacedBy: [],
        type: 'layout',
    ...
    create(context) {
         ...
         */
        function findOpeningParenOfParams(node: Node<'ArrowFunctionExpression'>) {
         ...
         * It is assumed that the given arrow function has parens of parameters and that it has exactly one
         * parameter.
         * @param node `ArrowFunctionExpression` node.
         * @returns the closing paren of parameters.
         */
        function getClosingParenOfParams(node: Node<'ArrowFunctionExpression'>) {
         ...
         */
        function hasCommentsInParensOfParams(
         ...
         * Only tokens within the range of the arrow function (tokens that are part of the arrow function) are taken
         * into account.
         * Example: <T>(a) => b
         * @param node `ArrowFunctionExpression` node.
         * @param openingParen Opening paren of parameters.
         ...
         */
        function hasUnexpectedTokensBeforeOpeningParen(
            node: Node<'ArrowFunctionExpression'>,
            openingParen: Token,
        ) {
        ...
        return {
            'ArrowFunctionExpression[params.length=1]':
                function onArrowFunctionExpressionParamsLength1(
                    node: Node<'ArrowFunctionExpression'>,
                ) {
