/**
 * @file Rule to flag comparisons to the value NaN
 ...
 * @returns `true` if the node is 'NaN' identifier.
 */
function isNaNIdentifier(node: Node | null) {
    return (
        Boolean(node)
...
const rule: LegacyRule<[{ enforceForSwitchCase?: boolean; enforceForIndexOf?: boolean }?]> = {
    meta: {
        type: 'problem',
    ...
    create(context) {
         ...
         * @param node The node to check.
         */
        function checkBinaryExpression(node: Node<'BinaryExpression'>) {
            if (
        ...
        /**
         * Checks the discriminant and all case clauses of the given `SwitchStatement` node for `switch(NaN)` and
         * `case NaN:`
         * @param node The node to check.
         */
        function checkSwitchStatement(node: Node<'SwitchStatement'>) {
         ...
         * @param node The node to check.
         */
        function checkCallExpression(node: Node<'CallExpression'>) {
