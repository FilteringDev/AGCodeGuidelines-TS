/**
 * @file Rule to flag declared but unused private class members
...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
        const trackedClasses: Map<
            string,
            {
                declaredNode: Node<'MethodDefinition'> | Node<'PropertyDefinition'>;
                isAccessor: boolean;
                isUsed?: boolean;
            }
        >[] = [];
         ...
         */
        function isWriteOnlyAssignment(privateIdentifierNode: Node<'PrivateIdentifier'>) {
        ...
        return {
            ...
            ClassBody(classBodyNode) {
                ...
                classBodyNode.body.forEach((bodyMember) => {
                    if (
             ...
             * @param privateIdentifierNode The private identifier node value.
             */
            PrivateIdentifier(privateIdentifierNode) {
                const classBody = trackedClasses.find((members) => members.has(privateIdentifierNode.name));
                ...
                /**
                 * We can't delete the memberDefinition, as we need to keep track of which member we are marking as
                 * used.
                 * In the case of nested classes, we only mark the first member we encounter as used. If you were to
                 * delete
                 * the member, then any subsequent usage could incorrectly mark the member of an encapsulating
                 * parent class
            ...
            'ClassBody:exit': function onClassBodyExit() {
                ...
                Array.from(unusedPrivateMembers!.entries()).forEach(
                    ([classMemberName, { declaredNode, isUsed }]) => {
                        if (isUsed) {
                            return;
                        }
                        context.report({
