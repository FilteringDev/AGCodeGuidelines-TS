/**
 * @file Rule to check for implicit global variables, functions and classes.
 * @author Joshua Peek
 */
...
const rule: LegacyRule<[{ lexicalBindings?: boolean }?]> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
         ...
         */
        function report(node: Node, messageId: string, kind?: string | undefined) {
            context.report({
                node,
                messageId,
                data: {
        ...
        return {
            Program(node: Node<'Program'>) {
                ...
                // The program's global scope carries the legacy implicit-variable collection.
                (
                    scope as Scope & { implicit: { variables: Variable[] } }
                ).implicit.variables.forEach((variable: Variable) => {
