/**
 * @file Rule to disallow unnecessary computed property keys in object literals
...
function hasUselessComputedKey(
    node: Node<'Property' | 'MethodDefinition' | 'PropertyDefinition'>,
) {
    if (!node.computed) {
        return false;
    }
...
const rule: LegacyRule<[{ enforceForClassMembers?: boolean }?]> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
         ...
         */
        function check(node: Node<'Property' | 'MethodDefinition' | 'PropertyDefinition'>) {
            if (hasUselessComputedKey(node)) {
                ...
                context.report({
                    ...
                    fix(fixer: Fixer) {
                        ...
                        // Insert a space before the key to avoid changing identifiers, e.g. ({ get[2]() {} }) to ({
                        // get2() {} })
        ...
        /**
         * A no-op function to act as placeholder for checking a node when the `enforceForClassMembers` option is
         * `false`.
         */
        function noop() {}
