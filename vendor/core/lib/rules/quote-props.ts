/**
 * @file Rule to flag non-quoted property names in object literals.
 * @author Mathias Bynens <http://mathiasbynens.be/>
 * @deprecated in ESLint v8.53.0
 */
import dependency0 from '../../compat/espree';
import dependency1 from './utils/ast-utils';
import dependency2 from './utils/keywords';
...
const espree = dependency0;
const astUtils = dependency1;
const keywords = dependency2;
...
const rule: LegacyRule<
    ...
    | [
          ('always' | 'as-needed' | 'consistent' | 'consistent-as-needed')?,
          { keywords?: boolean; unnecessary?: boolean; numbers?: boolean }?,
    ]
> = {
    meta: {
        deprecated: true,
    ...
    create(context) {
         ...
         */
        function isKeyword(tokenStr: string) {
        ...
        function areQuotesRedundant(
            rawKey: string,
            tokens: ReturnType<typeof espree.tokenize>,
            skipNumberLiterals?: boolean,
        ) {
            return (
                tokens.length === 1
                && tokens[0]!.start === 0
         ...
         */
        function getUnquotedKey(key: Node) {
         ...
         * @returns A replacement string for this property
         */
        function getQuotedKey(key: Node) {
         ...
         */
        function checkUnnecessaryQuotes(node: Node<'Property'>) {
            const { key } = node;
            ...
            node.properties.forEach((property) => {
                ...
                if (key.type === 'Literal' && typeof key.value === 'string') {
                    ...
                    if (checkQuotesRedundancy) {
                        ...
                        necessaryQuotes = necessaryQuotes
