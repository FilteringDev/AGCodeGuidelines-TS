/**
 * @file Rule to flag no-unneeded-ternary
...
const OPERATOR_INVERSES: Record<string, string> = {
    '==': '!=',
    '!=': '==',
    '===': '!==',
    '!==': '===',
...
const rule: LegacyRule<[{ defaultAssignment?: boolean }?]> = {
    meta: {
        type: 'suggestion',
    ...
    create(context) {
        ...
        /**
         * Creates an expression that represents the boolean inverse of the expression represented by the original
         * node
         * @param node A node representing an expression
         * @returns A string representing an inverted expression
         */
        function invertExpression(node: Node) {
            if (
         ...
         */
        function matchesDefaultAssignment(node: Node<'ConditionalExpression'>) {
            return (
            ...
            ConditionalExpression(node: Node<'ConditionalExpression'>) {
                if (isBooleanLiteral(node.alternate) && isBooleanLiteral(node.consequent)) {
                    context.report({
                        ...
                        fix(fixer: Fixer) {
                            if (node.consequent.value === node.alternate.value) {
                                // Replace `foo ? true : true` with just `true`, but don't replace `foo() ? true :
                                // true`
                            ...
                            // Replace `foo ? true : false` with `foo` if `foo` is guaranteed to be a boolean, or
                            // `!!foo` otherwise.
                    ...
                    context.report({
                        ...
                        fix(fixer: Fixer) {
                            const shouldParenthesizeAlternate = (astUtils.getPrecedence(node.alternate) < OR_PRECEDENCE
