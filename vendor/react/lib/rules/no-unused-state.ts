...
/**
 * @file  Attempts to discover all state fields in a React component and
...
interface ClassInfo {
    stateFields: Set<Node>;
    usedStateFields: Set<string | null>;
    aliases: Set<string | null> | null;
}
/**
 * @returns The result of this check.
 */
function getInitialClassInfo(): ClassInfo {
    return {
...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
         ...
         * @param node The node to inspect.
         */
        function isStateParameterReference(node: Node) {
            const classMethods = [
         ...
         * @param initialNode The initial node value.
         */
        function isStateReference(initialNode: Node) {
            let node = initialNode;
...
&& classInfo!.aliases.has(node.name);
         ...
         * @param node The node to inspect.
         */
        function addStateFields(node: Node) {
         ...
         * @param node The node to inspect.
         */
        function addUsedStateField(node: Node) {
            if (!classInfo) {
                return;
            }
         ...
         * @param node The node to inspect.
         */
        function handleStateDestructuring(node: Node) {
         ...
         */
        function handleAssignment(left: Node, right: Node) {
        ...
        function reportUnusedFields() {
            ...
            classInfo!.stateFields.forEach((node: Node) => {
         ...
         * @param node The node to inspect.
         */
        function handleES6ComponentEnter(node: Node) {
        ...
        return {
            ...
            'ClassProperty:exit': function onClassPropertyExit(node: Node<'ClassProperty'>) {
                if (
                    classInfo
                    && !node.static
                    && node.value
            ...
            'PropertyDefinition, ClassProperty': function onPropertyDefinitionClassProperty(
                node: Node<'PropertyDefinition' | 'ClassProperty'>,
            ) {
                if (!isGDSFP(node)) {
                    return;
                }
            ...
            'PropertyDefinition:exit': function onPropertyDefinitionExit(
            ...
            'MethodDefinition:exit': function onMethodDefinitionExit() {
                if (!classInfo) {
                    return;
                }
                // Forget our set of local aliases.
            ...
            'ExperimentalSpreadProperty, SpreadElement':
                function onExperimentalSpreadPropertySpreadElement(node: Node) {
                    if (classInfo && isStateReference(node.argument!)) {
                        classInfo = null;
                    }
                },
