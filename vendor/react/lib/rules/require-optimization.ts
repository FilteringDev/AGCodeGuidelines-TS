import dependency0 from 'object.values';
import type { Scope, LegacyRule, Node } from '../../types';
import type { Component } from '../../component-types';
/**
 * @file Enforce React components to have a shouldComponentUpdate method
 * @author Evgueni Naverniouk
 */
import dependency1 from '../util/Components';
import dependency2 from '../util/componentUtil';
import dependency3 from '../util/docsUrl';
import dependency4 from '../util/report';
import dependency5 from '../util/eslint';

const values = dependency0;

const Components = dependency1;
const componentUtil = dependency2;
const docsUrl = dependency3;
const report = dependency4;
const { getScope } = dependency5;

const messages = {
    noShouldComponentUpdate: 'Component is not optimized. Please add a shouldComponentUpdate method.',
};

const rule: LegacyRule<[{ allowDecorators?: string[] }?]> = {
    meta: {
        docs: {
            description: 'Enforce React components to have a shouldComponentUpdate method',
            category: 'Best Practices',
            recommended: false,
            url: docsUrl('require-optimization'),
        },

        messages,

        schema: [
            {
                type: 'object',
                properties: {
                    allowDecorators: {
                        type: 'array',
                        items: {
                            type: 'string',
                        },
                    },
                },
                additionalProperties: false,
            },
        ],
    },

    create: Components.detect((context, components) => {
        const configuration = context.options[0] || {};
        const allowDecorators = configuration.allowDecorators || [];

        /**
         * Checks to see if our component is decorated by PureRenderMixin via reactMixin
         * @param node The AST node being checked.
         * @returns True if node is decorated with a PureRenderMixin, false if not.
         */
        function hasPureRenderDecorator(node: Node) {
            if (node.decorators && node.decorators.length) {
                for (let i = 0, l = node.decorators.length; i < l; i += 1) {
                    if (
                        node.decorators[i]!.expression
                        && node.decorators[i]!.expression.callee
                        && node.decorators[i]!.expression.callee!.object
                        && node.decorators[i]!.expression.callee!.object!.name === 'reactMixin'
                        && node.decorators[i]!.expression.callee!.property
                        && node.decorators[i]!.expression.callee!.property!.name === 'decorate'
                        && node.decorators[i]!.expression.arguments
                        && node.decorators[i]!.expression.arguments!.length
                        && node.decorators[i]!.expression.arguments![0]!.name === 'PureRenderMixin'
                    ) {
                        return true;
                    }
                }
            }

            return false;
        }

        /**
         * Checks to see if our component is custom decorated
         * @param node The AST node being checked.
         * @returns True if node is decorated name with a custom decorated, false if not.
         */
        function hasCustomDecorator(node: Node) {
            const allowLength = allowDecorators.length;

            if (allowLength && node.decorators && node.decorators.length) {
                for (let i = 0; i < allowLength; i += 1) {
                    for (let j = 0, l = node.decorators.length; j < l; j += 1) {
                        const { expression } = node.decorators[j]!;
                        if (expression && expression.name === allowDecorators[i]) {
                            return true;
                        }
                    }
                }
            }

            return false;
        }

        /**
         * Checks if we are declaring a shouldComponentUpdate method
         * @param node The AST node being checked.
         * @returns True if we are declaring a shouldComponentUpdate method, false if not.
         */
        function isSCUDeclared(node: Node) {
            return !!node && node.name === 'shouldComponentUpdate';
        }

        /**
         * Checks if we are declaring a PureRenderMixin mixin
         * @param node The AST node being checked.
         * @returns True if we are declaring a PureRenderMixin method, false if not.
         */
        function isPureRenderDeclared(node: Node<'Property' | 'ObjectProperty' | 'ObjectMethod'>) {
            let hasPR = false;
            if (node.value && node.value.elements) {
                for (let i = 0, l = node.value.elements.length; i < l; i += 1) {
                    if (node.value.elements[i] && node.value.elements[i]!.name === 'PureRenderMixin') {
                        hasPR = true;
                        break;
                    }
                }
            }

            return !!node && node.key!.name === 'mixins' && hasPR;
        }

        /**
         * Mark shouldComponentUpdate as declared
         * @param node The AST node being checked.
         */
        function markSCUAsDeclared(node: Node) {
            components.set(node, {
                hasSCU: true,
            });
        }

        /**
         * Reports missing optimization for a given component
         * @param component The component to process
         */
        function reportMissingOptimization(component: Component) {
            report(context, messages.noShouldComponentUpdate, 'noShouldComponentUpdate', {
                node: component.node,
            });
        }

        /**
         * Checks if we are declaring function in class
         * @param node The value to inspect.
         * @returns True if we are declaring function in class, false if not.
         */
        function isFunctionInClass(node: Node) {
            let blockNode;
            let scope: Scope | null = getScope(context, node);
            while (scope) {
                blockNode = scope.block;
                if (blockNode && blockNode.type === 'ClassDeclaration') {
                    return true;
                }
                scope = scope.upper;
            }

            return false;
        }

        return {
            ArrowFunctionExpression(node: Node<'ArrowFunctionExpression'>) {
                // Skip if the function is declared in the class
                if (isFunctionInClass(node)) {
                    return;
                }
                // Stateless Functional Components cannot be optimized (yet)
                markSCUAsDeclared(node);
            },

            ClassDeclaration(node: Node<'ClassDeclaration'>) {
                if (
                    !(
                        hasPureRenderDecorator(node)
                        || hasCustomDecorator(node)
                        || componentUtil.isPureComponent(node, context)
                    )
                ) {
                    return;
                }
                markSCUAsDeclared(node);
            },

            FunctionDeclaration(node: Node<'FunctionDeclaration'>) {
                // Skip if the function is declared in the class
                if (isFunctionInClass(node)) {
                    return;
                }
                // Stateless Functional Components cannot be optimized (yet)
                markSCUAsDeclared(node);
            },

            FunctionExpression(node: Node<'FunctionExpression'>) {
                // Skip if the function is declared in the class
                if (isFunctionInClass(node)) {
                    return;
                }
                // Stateless Functional Components cannot be optimized (yet)
                markSCUAsDeclared(node);
            },

            MethodDefinition(node: Node<'MethodDefinition'>) {
                if (!isSCUDeclared(node.key)) {
                    return;
                }
                markSCUAsDeclared(node);
            },

            ObjectExpression(node: Node<'ObjectExpression'>) {
                // Search for the shouldComponentUpdate declaration
                const found = node.properties.some(
                    (property) => property.key && (isSCUDeclared(property.key) || isPureRenderDeclared(property)),
                );
                if (found) {
                    markSCUAsDeclared(node);
                }
            },

            'Program:exit': function onProgramExit() {
                // Report missing shouldComponentUpdate for all components
                values(components.list())
                    .filter((component) => !component.hasSCU)
                    .forEach((component) => {
                        reportMissingOptimization(component);
                    });
            },
        };
    }),
};

export default rule;
