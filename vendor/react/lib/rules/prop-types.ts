...
/**
 * @file Prevent missing props validation in a React component definition
...
const rule: LegacyRule<[{ ignore?: string[]; customValidators?: string[]; skipUndeclared?: boolean }?]> = {
    meta: {
        docs: {
    ...
    create: Components.detect((context, components) => {
         ...
         * @param initialDeclaredPropTypes Description of propTypes declared in the current component
         * @param keyList Dot separated name of the prop to check.
         * @returns True if the prop is declared, false if not.
         */
        function internalIsDeclaredInComponent(
            initialDeclaredPropTypes: Record<string, PropType> | undefined,
            keyList: string[],
        ): boolean {
            let declaredPropTypes = initialDeclaredPropTypes;

            for (let i = 0, j = keyList.length; i < j; i += 1) {
                const key = keyList[i]!;
                const { __ANY_KEY__: anyKey } = declaredPropTypes || {};
                const propType = declaredPropTypes
                ...
                if (
                    (propType.children as unknown) === true
                        || propType.containsUnresolvedSpread
                        || propType.containsIndexers
                ) {
                    return true;
                }
                // Legacy validators may expose an explicit accepted-property map.
                const { acceptedProperties } = propType as PropType & {
                    acceptedProperties?: Record<string, unknown>;
                };
                if (acceptedProperties) {
                    return key! in acceptedProperties;
                }
                if (propType.type === 'union') {
                    ...
                    const unionPropType: Record<string, PropType> = {};
         ...
         * @param initialNode The AST node being checked.
         * @param names List of names of the prop to check.
         * @returns True if the prop is declared, false if not.
         */
        function isDeclaredInComponent(initialNode: Node, names: string[]) {
            let node = initialNode;

            while (node) {
                const component = components.get(node);

                const isDeclared = component
         ...
         * @param component The component to process
         */
        function reportUndeclaredPropTypes(component: Component) {
        ...
        /**
         * @param component The current component to process
         * @param list The all components to process
         * @returns True if the component is nested False if not.
         */
        function checkNestedComponent(component: Component, list: Component[]) {
            const componentIsMemo = component.node.callee && component.node.callee.name === 'memo';
            const argumentIsForwardRef = component.node.arguments
