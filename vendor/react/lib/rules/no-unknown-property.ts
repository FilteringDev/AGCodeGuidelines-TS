...
/**
 * @file Prevent usage of unknown DOM property
...
const ATTRIBUTE_TAGS_MAP: Record<string, string[]> = {
    abbr: ['th', 'td'],
    charset: ['meta'],
    checked: ['input'],
    ...
    noModule: ['script'],
    // Media events allowed only on audio and video tags, see
    // https://github.com/facebook/react/blob/256aefbea144/CHANGELOG.md#notable-enhancements
    onAbort: ['audio', 'video'],
    onCancel: ['dialog'],
...
const SVGDOM_ATTRIBUTE_NAMES: Record<string, string> = {
 ...
 */
function getDOMPropertyNames(context: RuleContext) {
        ...
        testReactVersion(context, '>= 16.1.0')
            ? ([] as string[]).concat(
                testReactVersion(context, '>= 16.4.0')
                    ? ([] as string[]).concat(
                        // these were added in React v16.4.0, see
                        // https://reactjs.org/blog/2018/05/23/react-v-16-4.html and
                        // https://github.com/facebook/react/pull/12507
                        REACT_ON_PROPS,
                        testReactVersion(context, '>= 19')
                            ? [
                                // precedence was added in React v19, see
 ...
 */
function isValidHTMLTagInJSX(childNode: Node<'JSXAttribute'>) {
    const tagConvention = /^[a-z][^-]*$/;
    if (tagConvention.test(childNode.parent.name.name as string)) {
        return !childNode.parent.attributes.some(
            (attrNode) => attrNode.type === 'JSXAttribute'
 ...
 * @returns Result
 */
function normalizeAttributeCase(name: string) {
    return (
        DOM_PROPERTIES_IGNORE_CASE.find((element) => element.toLowerCase() === name.toLowerCase())
        || name
    );
}
 ...
 * if the name starts with "data-" and has alphanumeric words (browsers require lowercase, but React and TS lowercase
 * them),
 * not start with any casing of "xml", and separated by hyphens (-) (which is also called "kebab case" or "dash
 * case"),
 * then the attribute is a valid data attribute.
 * @param name - Attribute name to be tested
 * @returns Result
 */
function isValidDataAttribute(name: string) {
 ...
 * @returns Result
 */
function hasUpperCaseCharacter(name: string) {
 ...
 * @returns The result of this check.
 */
function isValidAriaAttribute(name: string) {
 ...
 * @returns tag name
 */
function getTagName(node: Node<'JSXAttribute'>) {
 ...
 * @returns result
 */
function tagNameHasDot(node: Node<'JSXAttribute'>) {
 ...
 */
function getStandardName(name: string, context: RuleContext) {
    if (has(DOM_ATTRIBUTE_NAMES, name)) {
        return DOM_ATTRIBUTE_NAMES[name as keyof typeof DOM_ATTRIBUTE_NAMES];
    }
    if (has(SVGDOM_ATTRIBUTE_NAMES, name)) {
        return SVGDOM_ATTRIBUTE_NAMES[name as keyof typeof SVGDOM_ATTRIBUTE_NAMES];
    }
...
const rule: LegacyRule<[{ ignore?: string[]; requireDataLowercase?: boolean }?]> = {
    meta: {
        docs: {
    ...
    create(context) {
        ...
        return {
            JSXAttribute(node: Node<'JSXAttribute'>) {
                ...
                // Let's dive deeper into tags that are HTML/DOM elements (`<button>`), and not React components
                // (`<Button />`)

                // Some attributes are allowed on some tags only
                const allowedTags = has(ATTRIBUTE_TAGS_MAP, name)
                    ? ATTRIBUTE_TAGS_MAP[name as keyof typeof ATTRIBUTE_TAGS_MAP]
                    : null;
                if (tagName && allowedTags) {
                    // Scenario 1A: Allowed attribute found where not supposed to, report it
                    if (allowedTags.indexOf(tagName as string) === -1) {
                ...
                if (hasStandardNameButIsNotUsed) {
                    // Scenario 2B: The name of the attribute is close to a standard one, report it with the standard
                    // name
                    report(
                        context,
