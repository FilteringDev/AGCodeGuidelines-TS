import type {
    RuleContext, Scope, Variable, LegacyRule, Node,
} from '../../types';
/**
 * @file Prevents jsx context provider values from taking values that
 ...
 * @param callScope The call scope value.
 * @returns The result of this check.
 */
function isConstruction(
    node: Node,
    callScope: Scope,
): { type: string; node: Node; usage?: Node } | null {
    switch (node.type) {
        case 'Literal':
            if (node.regex != null) {
        ...
        case 'ConditionalExpression':
            return (
                isConstruction(node.consequent, callScope) || isConstruction(node.alternate, callScope)
            );
 ...
 * @returns The result of this check.
 */
function isReactContext(context: RuleContext, node: Node<'JSXIdentifier'>) {
    let scope: Scope | null = getScope(context, node);
    let variableScoping: Variable | null | undefined = null;
