 ...
 */
function isMultilineProp(node: Node) {
 ...
 * @returns The result of this check.
 */
function isReservedPropName(name: unknown, list: readonly unknown[]) {
    return list.indexOf(name) >= 0;
}

let attributeMap: WeakMap<Node, { end: number; hasComment: boolean }>;
// attributeMap = { end: endrange, hasComment: true||false if comment in between nodes exists, it needs to be sorted
// to end }
 ...
 * @returns The result of this check.
 */
function shouldSortToEnd(node: Node) {
...
function contextCompare(
    a: Node<'JSXAttribute' | 'JSXSpreadAttribute'>,
    b: Node<'JSXAttribute' | 'JSXSpreadAttribute'>,
    options: Options & { reservedList: readonly unknown[] },
) {
 ...
 * @param attributes The value to inspect.
 * @param context The context of the rule
 * @returns The result of this check.
 */
function getGroupsOfSortableAttributes(
    attributes: Array<Node<'JSXSpreadAttribute'> | Node<'JSXAttribute'>>,
    context: RuleContext,
) {
    const sourceCode = getSourceCode(context);

    const sortableAttributeGroups: Node<'JSXAttribute' | 'JSXSpreadAttribute'>[][] = [];
    let groupCount = 0;
    /**
     * @param attribute The attribute value.
     */
    function addtoSortableAttributeGroups(attribute: Node<'JSXAttribute' | 'JSXSpreadAttribute'>) {
...
function generateFixerFunction(
    ...
    return function fixFunction(fixer: Fixer) {
        const fixers: ReturnType<Fixer['replaceTextRange']>[] = [];
 ...
 * @returns If an error is detected, a function to generate the error message, otherwise, `undefined`
 */

/**
 * @param context The rule context.
 * @param reservedFirst The reserved first value.
 * @returns The result of this check.
 */
function validateReservedFirstConfig(context: RuleContext, reservedFirst: boolean | unknown[]) {
    if (reservedFirst) {
        if (Array.isArray(reservedFirst)) {
            ...
            if (reservedFirst.length === 0) {
                return function Report(decl: Node) {
...
const reportedNodeAttributes = new WeakMap<Node, string[]>();
/**
...
function reportNodeAttribute(
    nodeAttribute: Node<'JSXAttribute' | 'JSXSpreadAttribute'>,
    errorType: keyof typeof messages,
    node: Node<'JSXOpeningElement'>,
    context: RuleContext<[Options?]>,
    reservedList: readonly unknown[],
) {
    ...
    create(context) {
        ...
        return {
            ...
            JSXOpeningElement(node: Node<'JSXOpeningElement'>) {
                ...
                const nodeReservedList = reservedFirst && !jsxUtil.isDOMComponent(node)
