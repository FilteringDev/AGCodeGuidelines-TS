...
/**
 * @file Common propTypes detection functionality.
 */
import dependency1 from './annotations';
import dependency2 from './props';
import dependency3 from './variable';
import dependency4 from './version';
import dependency5 from './propWrapper';
import dependency6 from './ast';
import dependency7 from './isFirstLetterCapitalized';
import dependency8 from './eslint';
import type {
    RuleContext, Node, Variable, SourceCode,
} from '../../types';

const flatMap = dependency0;

const annotations = dependency1;
const propsUtil = dependency2;
const variableUtil = dependency3;
const { testFlowVersion } = dependency4;
const propWrapperUtil = dependency5;
const astUtil = dependency6;
const isFirstLetterCapitalized = dependency7;
 ...
 */
function isFunctionType(node: Node) {
    if (!node) {
        return false;
    }
 ...
 * @returns True if the node is a class with generic prop types, false if not.
 */
function isSuperTypeParameterPropsDeclaration(node: Node) {
 ...
 * @param context Array of properties to iterate.
 * @param properties Array of properties to iterate.
 * @param fn Function to call on each property, receives property key
 * and property value. (key, value) => void
 * @param [handleSpreadFn] Function to call on each ObjectTypeSpreadProperty, receives the
 * argument
 */
function iterateProperties(
    context: RuleContext,
    properties: Node[] | undefined,
    fn: (key: string, value: Node, property: Node) => void,
    handleSpreadFn?: (argument: Node) => void,
) {
    if (properties && properties.length && typeof fn === 'function') {
        for (let i = 0, j = properties.length; i < j; i += 1) {
            const node = properties[i]!;
            const key = astUtil.getKeyValue(context, node) as string;
 ...
 * @param node the AST node being checked.
 * @returns True if the node has a ClassBody ancestor, false if not.
 */
function isInsideClassBody(node: Node) {
    let { parent } = node;
    while (parent) {
 ...
 * @returns The result of this check.
 */
function startWithCapitalizedLetter(node: Node) {
 ...
 * @returns The result of this check.
 */
export default function propTypesInstructions(
    context: RuleContext,
    ...
    let stack: Record<string, Node>[];

    const classExpressions: Node[] = [];
    const defaults = { customValidators: [] as string[] };
    const configuration = {
        ...defaults,
        ...((context.options[0] as { customValidators?: string[] } | undefined) || {}),
    };
    ...
    const genericTypeParamIndexWherePropsArePresent: Record<string, number> = {
        ComponentProps: 0,
    ...
    const genericReactTypesImport = new Set<string>();
    // import { FC as X } from 'react' -> localToImportedMap = { x: FC }
    const localToImportedMap: Record<string, string> = {};
     ...
     * @param key The name of the identifier to access.
     * @returns The ASTNode associated with the given identifier.
     */
    function getInTypeScope(key: string) {
     ...
     * @returns The ASTNode associated with the given identifier.
     */
    function setInTypeScope(key: string, value: Node) {
     ...
     * @param validator Name of validator to check.
     * @returns True if validator should be checked by custom validator.
     */
    function hasCustomValidator(validator: string) {
        return customValidators.indexOf(validator) !== -1;
    }

    const typeDeclarationBuilders: TypeDeclarationBuilders & {
        build(annotation: Node, parentName: string, seen?: Set<Node>): PropType;
    } = {
        /**
         * Creates the representation of the React props type annotation for the component.
         * The representation is used to verify nested used properties.
         * @param annotation Type annotation for the props class property.
         * @param parentName The value to inspect.
         * @param [initialSeen] The value to inspect.
         * @returns The representation of the declaration, empty object means
         *    the property is declared without the need for further analysis.
         */
        build(annotation: Node, parentName: string, initialSeen?: Set<Node>): PropType {
            ...
            if (annotation.type in typeDeclarationBuilders) {
                // The table key and annotation tag are checked together at this dispatch boundary.
                return (
                    typeDeclarationBuilders[annotation.type as keyof TypeDeclarationBuilders] as (
                        node: Node,
                        parent: string,
                        visited: Set<Node>,
                    ) => PropType
                )(annotation, parentName, seen);
            }
            return {};
        },
        ...
        GenericTypeAnnotation(annotation, parentName, seen) {
            if (getInTypeScope(annotation.id.name!)) {
                return typeDeclarationBuilders.build(
        ...
        ObjectTypeAnnotation(annotation, parentName, seen) {
            ...
            const shapeTypeDefinition: Extract<PropType, { type: 'shape' | 'exact' | 'object' }> = {
                type: 'shape',
                children: {},
            };
            iterateProperties(
                ...
                (childKey, childValue, propNode) => {
                    ...
                    if (childKey || childValue) {
                        const types = typeDeclarationBuilders.build(childValue, fullName, seen);
                ...
                (spreadNode) => {
                    const key = astUtil.getKeyValue(context, spreadNode) as string;
                    const types = typeDeclarationBuilders.build(spreadNode, key, seen);
                    if (!types.children) {
            ...
            // Mark if this shape has spread or an indexer. We will know to consider all props from this shape as
            // having propTypes,
            ...
            // Deprecated: containsSpread is not used anymore in the codebase, ensure to keep API backward
            // compatibility
        ...
        UnionTypeAnnotation(annotation, parentName, seen) {
            const unionTypeDefinition: UnionTypeDefinition = {
                type: 'union',
                children: annotation.types.map((type) => typeDeclarationBuilders.build(type, parentName, seen)),
            };
        ...
        ArrayTypeAnnotation(annotation, parentName, seen) {
            ...
            const child = typeDeclarationBuilders.build(annotation.elementType, fullName, seen);
     ...
     * @param node The annotation or a node containing the type annotation.
     * @returns The resolved type annotation for the node.
     */
    function resolveTypeAnnotation(node: Node) {
        let annotation = (node.left && node.left.typeAnnotation) || node.typeAnnotation || node;
        while (
            annotation
            && (annotation.type === 'TypeAnnotation' || annotation.type === 'NullableTypeAnnotation')
        ) {
     ...
     * @param propTypes The value to inspect.
     * @param declaredPropTypes The value to inspect.
     * @returns True if propTypes should be ignored (e.g. when a type can't be resolved, when it is imported)
     */
    function declarePropTypesForObjectTypeAnnotation(
        ...
        iterateProperties(
            ...
            (key, value, propNode) => {
                ...
                const types = typeDeclarationBuilders.build(value, key);
                types.fullName = key;
                types.name = key;
                types.node = propNode;
                types.isRequired = !propNode.optional;
                Object.assign(declaredPropTypes, { [key]: types });
            },
            (spreadNode) => {
                const key = astUtil.getKeyValue(context, spreadNode) as string;
                ...
                if (!spreadAnnotation) {
                    ...
                    const spreadIgnoreValidation = declarePropTypesForObjectTypeAnnotation(
                        spreadAnnotation,
                        declaredPropTypes,
                    );
    ...
    function declarePropTypesForIntersectionTypeAnnotation(
        propTypes: Node<'IntersectionTypeAnnotation'>,
        declaredPropTypes: Record<string, PropType>,
    ): boolean {
     ...
     * @param node The value to inspect.
     * @param rootNode The value to inspect.
     * @param callback called with the resolved value only if resolved.
     */
    function resolveValueForIdentifierNode(
        node: Node | null | undefined,
        rootNode: Node,
        callback: (value: Node | null | undefined) => void,
    ) {
        if (rootNode && node && node.type === 'Identifier') {
            ...
            const identVariable = scope.variableScope.variables.find(
                (variable: Variable) => variable.name === node.name,
            );
            if (identVariable) {
     ...
     * The representation is used to verify nested used properties.
     * @param initialValue Node of the PropTypes for the desired property
     * @param parentName The value to inspect.
     * @param rootNode The value to inspect.
     * @returns The representation of the declaration, empty object means
     *    the property is declared without the need for further analysis.
    ...
    function buildReactDeclarationTypes(
        initialValue: Node | null | undefined,
        parentName: string,
        rootNode: Node,
    ): PropType {
        ...
        if (
            ...
            switch (callName) {
                ...
                case 'exact': {
                    ...
                    const shapeTypeDefinition: Extract<
                        PropType,
                        { type: 'shape' | 'exact' | 'object' }
                    > = {
                        type: callName,
                        children: {},
                    };
                ...
                case 'oneOfType': {
                    ...
                    const unionTypeDefinition: UnionTypeDefinition = {
                        type: 'union',
                        children: argument!.elements.map(
                            (element) => buildReactDeclarationTypes(element, parentName, rootNode),
                        ),
                    };
                    if (unionTypeDefinition.children.length === 0) {
                        // no complex type found, simply accept everything
     ...
     */
    function isValidReactGenericTypeAnnotation(annotation: Node) {
     ...
     * @returns The result of this check.
     */
    function getLeftMostTypeName(node: Node): string | undefined {
        if (node.name) {
     ...
     * @param node The node to inspect.
     */
    function getRightMostTypeName(node: Node): string | undefined {
        if (node.name) {
            return node.name as string | undefined;
        }
        if (node.right) {
     ...
     * @returns The result of this check.
     */
    function filterInterfaceOrTypeAlias(node: Node) {
     ...
     * @param node The value to inspect.
     * @param typeName The value to inspect.
     * @returns The result of this check.
     */
    function filterInterfaceOrAliasByName(node: Node, typeName: string) {
        return (
    ...
    class DeclarePropTypesForTSTypeAnnotation {
        propsAnnotation: Node;

        declaredPropTypes: Record<string, PropType>;

        foundDeclaredPropertiesList: Node[];

        referenceNameMap: Set<string>;

        sourceCode: SourceCode;

        shouldIgnorePropTypes: boolean;

        rootNode: Node;

        shouldSpecifyOptionalChildrenProps?: boolean;

        shouldSpecifyClassNameProp?: boolean;

        constructor(propTypes: Node, declaredPropTypes: Record<string, PropType>, rootNode: Node) {
            this.propsAnnotation = propTypes;
            ...
            this.visitTSNode(this.propsAnnotation);
        ...
        visitTSNode(node: Node) {
            ...
            if (astUtil.isTSTypeAnnotation(node)) {
                ...
                if (Array.isArray(node.members)) {
                    this.foundDeclaredPropertiesList = this.foundDeclaredPropertiesList.concat(
                        node.members,
                    );
                }
         ...
         * @param node The value to inspect.
         */
        searchDeclarationByName(node: Node) {
            let typeName;
            if (astUtil.isTSTypeReference(node)) {
                ...
                if (
                    ...
                    && nodeTypeArguments
                    && (nodeTypeArguments as Node & { length?: number }).length !== 0
                ) {
            ...
            const declarations = flatMap<Node, Node>(
                candidateTypes,
         ...
         * @param node The value to inspect.
         */
        traverseDeclaredInterfaceOrTypeAlias(node: Node) {
            if (astUtil.isTSInterfaceDeclaration(node)) {
                // Handle TSInterfaceDeclaration interface Props { name: string, id: number}, should put in properties
                // list directly;
                this.foundDeclaredPropertiesList = this.foundDeclaredPropertiesList.concat(
                    (node.body as Node<'TSInterfaceBody'>).body,
                );
            }
        ...
        convertIntersectionTypeToPropTypes(node: Node) {
            if (!node) {
                return;
            }
        ...
        convertReturnTypeToPropTypes(node: Node, rootNode: Node) {
            // ReturnType<T> should always have one parameter
            const nodeTypeArguments = propsUtil.getTypeArguments(node);
            if (nodeTypeArguments) {
                if (nodeTypeArguments.params!.length === 1) {
                    let returnType: Node | undefined = nodeTypeArguments.params![0];
                    // This line is trying to handle typescript-eslint-parser
                        ...
                        if (Array.isArray(returnTypeFunction)) {
                            ...
                            returnTypeFunction.forEach((func) => {
                                if (isFunctionType(func!)) {
                                    let res: Node | false | null | undefined = func!.body;
                                    ...
                                    switch ((res as Node).type) {
                                        case 'ObjectExpression':
                                            iterateProperties(
                                                context,
                                                (res as Node).properties,
                                                (key, value, propNode) => {
                                                    if (
                                                        propNode
                                                        && astUtil.isCallExpression(propNode.argument)
                                                    ) {
                                                        const propNodeTypeArguments = propsUtil.getTypeArguments(
                                                            propNode.argument,
                                                        );
                                                        if (propNodeTypeArguments) {
                                                            this.visitTSNode(propNodeTypeArguments);
                                                        } else {
                                                            // Ignore this CallExpression return value since it
                                                            // doesn't have any typeParameters to let us know it's
                                                            // types.
                                                            this.shouldIgnorePropTypes = true;
                                                            return;
                                                        }
                                            ...
                                            break;
                                        case 'CallExpression':
                                            if (propsUtil.getTypeArguments(res as Node)) {
                                                this.visitTSNode(
                                                    propsUtil.getTypeArguments(res as Node)!,
                                                );
                                            } else {
                                                // Ignore this CallExpression return value since it doesn't have any
                                                // typeParameters to let us know it's types.
                                                this.shouldIgnorePropTypes = true;
                                            }
                                            break;
                                        default:
        ...
        endAndStructDeclaredPropTypes() {
            ...
            this.foundDeclaredPropertiesList.forEach((tsInterfaceBody) => {
                if (
                    ...
                    let accessor: 'name' | 'raw' | 'value' = 'name';
                    ...
                    this.declaredPropTypes[tsInterfaceBody.key[accessor] as string] = {
                        fullName: tsInterfaceBody.key[accessor] as string,
                        name: tsInterfaceBody.key[accessor] as string,
     ...
     * @param node The AST node being checked.
     * @param initialPropTypes The AST node containing the proptypes
     * @param rootNode The value to inspect.
     */
    function markPropTypesAsDeclared(node: Node, initialPropTypes: Node | null, rootNode: Node) {
        let propTypes = initialPropTypes;
        ...
        switch (propTypes?.type) {
            ...
            case 'MemberExpression': {
                ...
                while (
                    ...
                    if (propName! in curDeclaredPropTypes) {
                        curDeclaredPropTypes = curDeclaredPropTypes[propName!]!.children as Record<
                            string,
                            PropType
                        >;
                ...
                if (propTypes && propTypes.parent && propTypes.property) {
                    ...
                    const parentProp = getText(context, propTypes.parent.left.object).replace(
                        /^.*\.propTypes\./,
                        '',
                    );
                    const types = buildReactDeclarationTypes(
                    ...
                    while (n) {
                        if (
                            ...
                            // Found a propType used inside of another propType. This is not considered usage, we'll
                            // still validate
                            // this component.
            ...
            case 'IntersectionTypeAnnotation':
                ignorePropsValidation = declarePropTypesForIntersectionTypeAnnotation(
                    propTypes as Node<'IntersectionTypeAnnotation'>,
                    declaredPropTypes,
                );
                break;
     ...
     * @param rootNode The value to inspect.
     */
    function markAnnotatedFunctionArgumentsAsDeclared(node: Node, rootNode: Node) {
        ...
        if (isNodeAnnotated) {
            ...
            // implements what's discussed here:
            // https://github.com/jsx-eslint/eslint-plugin-react/issues/2777#issuecomment-683944481
     ...
     * @returns The resolved type annotation for the node.
     */
    function resolveSuperParameterPropsType(node: Node) {
        ...
        try {
            ...
            // In case there is no flow version defined, we can safely assume that when there are 3 Props we are
            // dealing with version <= 0.52
            propsParameterPosition = parameters!.params.length <= 2 ? 0 : 1;
        }

        let annotation = parameters!.params[propsParameterPosition] as Node | undefined;
        while (
            annotation
            && (annotation.type === 'TypeAnnotation' || annotation.type === 'NullableTypeAnnotation')
        ) {
     ...
     */
    function isAnnotatedClassPropsDeclaration(node: Node) {
    ...
    return {
        ...
        MemberExpression(node: Node<'MemberExpression'>) {
            if (propsUtil.isPropTypesDeclaration(node)) {
                ...
                try {
                    ...
                    if (!(e instanceof RangeError) || e.constructor !== RangeError) {
                        throw e;
                    }
                }
            }
        ...
        TypeParameterDeclaration(node: Node<'TypeParameterDeclaration'>) {
        ...
        'Program:exit': function onProgramExit() {
            classExpressions.forEach((node: Node) => {
                if (isSuperTypeParameterPropsDeclaration(node)) {
                    markPropTypesAsDeclared(node, resolveSuperParameterPropsType(node)!, node);
                }
            });
