...
import { getFilename, getPhysicalFilename } from '../../utils/contextCompat';
import ExportMapBuilder from '../exportMap/builder';
import docsUrl from '../docsUrl';

const rule: LegacyRule<[{ commonjs?: boolean }?]> = {
    meta: {
        type: 'problem',
        docs: {
    ...
    create(context) {
        ...
        function checkSpecifiers(
            ...
            node: Node<'ImportDeclaration' | 'ExportNamedDeclaration'>,
        ) {
            ...
            const imports = ExportMapBuilder.get(node.source!.value as string, context);
            ...
            node.specifiers.forEach((im) => {
                ...
                const name = (im[key]!.name || im[key]!.value) as string;
         ...
         * @param node The node to inspect.
         */
        function checkRequire(node: Node) {
            if (
                !options.commonjs
            ...
            const variableExports = ExportMapBuilder.get(source!.value as string, context);
