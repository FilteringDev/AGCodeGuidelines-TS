...
const rule: LegacyRule<[]> = {
    ...
    create(context) {
        /**
         * Check default.
         * @param nameKey The name key value.
         * @param defaultSpecifier The default specifier value.
         */
        function checkDefault(
            nameKey: 'local' | 'exported',
            defaultSpecifier: Node<'ImportDefaultSpecifier' | 'ExportDefaultSpecifier'>,
        ) {
            /**
            ...
            if (importedModule.reexports.has(analyzedName) && importedModule.reexports.has('default')) {
                const thingImportedWithNamedImport = importedModule.reexports
                    .get(analyzedName)!
                    .getImport();
                const thingImportedWithDefaultImport = importedModule.reexports
                    .get('default')!
                    .getImport();
                ...
                if (
                    ...
                    && (thingImportedWithNamedImport as { local?: string }).local
                        === (thingImportedWithDefaultImport as { local?: string }).local
                ) {
