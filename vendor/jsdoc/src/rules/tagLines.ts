...
type Options = [
    ('always' | 'any' | 'never')?,
    {
        ...
        tags?: Record<string, { lines?: 'always' | 'never' | 'any'; count?: number }>;
    }?,
];

/**
 * @param root0 The root0 value.
 * @param root0.maxBlockLines The max block lines value.
 * @param root0.startLines The start lines value.
 * @param root0.utils The utils value.
 *
 *
 *
 *  cfg
 ...
 */
const checkMaxBlockLines = (root0: {
...
export default iterateJsdoc(
    ({ context, jsdoc, utils }) => {
        ...
        jsdoc.tags.some((tg, tagIdx) => {
            ...
            let lastEmpty: null | import('../iterateJsdoc').Integer = null;

            let reportIndex: null | import('../iterateJsdoc').Integer = null;
            let emptyLinesCount = 0;
            Array.from(tg.source.entries()).forEach(([
                idx,
                {
                    tokens: {
                        description, end, name, tag, type,
                    },
                },
            ]) => {
                ...
                if (lineDiff < 0) {
                    const fixer = () => {
                        utils.removeTag(tagIdx, {
                            tagSourceOffset: (lastEmpty as import('../iterateJsdoc').Integer)
                                + lineDiff
                                + 1,
                        });
                    };
                    ...
                    const fixer = () => {
                        utils.addLines(
                            tagIdx,
                            lastEmpty as import('../iterateJsdoc').Integer,
            ...
            if (reportIndex !== null) {
                const fixer = () => {
                    utils.removeTag(tagIdx, {
                        tagSourceOffset: reportIndex as import('../iterateJsdoc').Integer,
                    });
                };
        ...
        (applyToEndTag ? jsdoc.tags : jsdoc.tags.slice(0, -1)).some(
            (tg, tagIdx) => {
                const lines: {
                    ...
                    number: import('../iterateJsdoc').Integer;
                }[] = [];
                ...
                const currentTg = (currentTag ? tags[currentTag.slice(1)] : undefined);
                const tagCount = currentTg?.count;

                const defaultAlways = alwaysNever === 'always'
                ...
                if (defaultAlways || overrideAlways) {
                    ...
                    const line = lines[lines.length - 1]?.number
                        || tg.source[tagSourceIdx]!.number;
                    utils.reportJSDoc(
        ...
        if (
            typeof startLines === 'number'
            || typeof startLinesWithNoTags === 'number'
        ) {
            ...
            if (trailingDiff > 0) {
                utils.reportJSDoc(
                    ...
                    () => {
                        utils.setBlockDescription(
                            (info, seedTokens, descLines, postDelims) => descLines
                                .slice(0, -trailingDiff)
                                .map((desc, idx) => ({
                                    number: 0,
                                    source: '',
                                    tokens: seedTokens({
