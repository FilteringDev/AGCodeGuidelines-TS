...
type Options = [{ allowEmptyNamepaths?: boolean }?];
...
const suppressTypes = new Set([
    ...
    // https://github.com/google/closure-compiler (master):
    // src/com/google/javascript/jscomp/parsing/ParserConfig.properties#L154
    'accessControls',
 ...
 */
const tryParsePathIgnoreError = (
    path: string,
    mode: import('jsdoc-type-pratt-parser').ParseMode | 'permissive',
): boolean => {
    try {
...
export default iterateJsdoc(
    ({
        ...
        const { allowEmptyNamepaths = false } = (context.options as Options)[0] || {};
        const { mode } = settings;

        (jsdoc.tags).forEach((tag) => {
            /**
             * @param namepath The namepath value.
             * @param [tagName] The tag name value.
             * @returns The result of this check.
             */
            const validNamepathParsing = function validNamepathParsing(
                namepath: string,
                tagName?: string,
            ): boolean {
                ...
                if (tagName) {
                    switch (tagName) {
                        ...
                        case 'memberof!': {
                            ...
                            if (['#', '.', '~'].includes(endChar)) {
                                handled = tryParsePathIgnoreError(
                                    namepath.slice(0, -1),
                                    mode,
                                );
                            }
             ...
             * @returns The result of this check.
             */
            const validTypeParsing = function validTypeParsing(type: string): boolean {
                let parsedTypes;
                try {
                    if (mode === 'permissive') {
                ...
                if (mode === 'closure' || mode === 'typescript') {
                    traverse(parsedTypes, (node) => {
                        ...
                        if (
                            (typ === 'JsdocTypeObjectField'
                                || typ === 'JsdocTypeKeyValue')
                            && node.right?.type === 'JsdocTypeNullable'
                            && node.right?.meta?.position === 'suffix'
                        ) {
                            report(
                                `Syntax error in type: ${node.right.type}`,
                                null,
                                tag,
                            );
                        }
            ...
            if (tag.problems.length) {
                const msg = tag.problems
                    .reduce((str, { message }) => `${str}; ${message}`, '')
                    .slice(2);
            ...
            if (tag.tag === 'borrows') {
                const thisNamepath = (
                    utils.getTagDescription(tag) as string
                )
                    .replace(asExpression, '')
                    .trim();

                if (
                    !asExpression.test(utils.getTagDescription(
                        tag,
                    ) as string)
                    || !thisNamepath
                ) {
                ...
                if (parsedTypes) {
                    traverse(parsedTypes, (node) => {
                        ...
                        if (type !== undefined && !suppressTypes.has(type)) {
                            report(
                                `Syntax error in suppress type: ${type}`,
                                null,
                                tag,
                            );
                        }
            ...
            const otherModeMaps = (
                [
                    ...
                    'closure',
                    'permissive',
                ] as import('../jsdocUtils').ParserMode[]
            )
                .filter((mde) => mde !== mode)
            ...
            if (tagMightHaveNamePosition !== true && tag.name) {
                const modeInfo = tagMightHaveNamePosition === false
                    ? ''
                    : ` in "${mode}" mode`;
                report(
                    `@${tag.tag} should not have a name${modeInfo}.`,
                    null,
                    tag,
                );
            ...
            if (tag.type.trim().startsWith('@')) {
                Object.assign(tag, { description: `{${tag.type}} ${tag.description}` });
                Object.assign(tag, { type: '' });
            }
            ...
            if (
                ...
                && !tag.name
                && !allowEmptyNamepaths
                && !['arg', 'argument', 'param', 'prop', 'property'].includes(
                    tag.tag,
                )
                && (tag.tag !== 'see'
                    || !utils.getTagDescription(tag).includes('{@link'))
            ) {
                const modeInfo = tagMustHaveNamePosition === true
                    ? ''
                    : ` in "${mode}" mode`;
                report(
            ...
            if (
                ...
                report(
                    `Tag @${tag.tag} must have a type${modeInfo}.`,
                    null,
                    tag,
                );
            ...
            if (
                tagMissingRequiredTypeOrNamepath !== false
                && !allowEmptyNamepaths
            ) {
                const modeInfo = tagMissingRequiredTypeOrNamepath === true
                    ? ''
                    : ` in "${mode}" mode`;
                report(
            ...
            if (
                hasTypePosition
                && (tag.type !== 'const' || tag.tag !== 'type')
            ) {
                validTypeParsing(tag.type);
            }

            // VALID NAME/NAMEPATH
            const hasNamepathPosition = (tagMustHaveNamePosition !== false
            ...
            (tag.inlineTags).forEach((inlineTag) => {
                if (
                    ...
                    report(
                        `Inline tag "${inlineTag.tag}" missing content`,
                        null,
                        tag,
                    );
                }
        ...
        (jsdoc.inlineTags).forEach((inlineTag) => {
            if (
                inlineTags.has(inlineTag.tag)
                && !inlineTag.text
                && !inlineTag.namepathOrURL
