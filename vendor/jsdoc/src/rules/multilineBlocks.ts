 ...
 * @param requireSingleLineUnderCount The require single line under count value.
 * @returns The result of this check.
 */
const checkForShortTags = (
    ...
    const tagLines = jsdoc.source.reduce(
        (
            acc,
            {
                tokens: {
    ...
    if (!hasMultiDescOrType && isUnderCountLimit && tagLines === 1) {
        const fixer = () => {
            const { tokens } = (jsdoc.source[lastLineWithTag]!);
            Object.assign(jsdoc, {
                source: [
                    {
                        number: 0,
 ...
 * @returns The result of this check.
 */
const checkForShortDescriptions = (
    jsdoc: import('@es-joy/jsdoccomment').JsdocBlockWithInline,
    utils: import('../iterateJsdoc').Utils,
    requireSingleLineUnderCount: number,
) => {
    ...
    let isUnderCountLimit = false;
    const descLines = jsdoc.source.reduce(
        (
            acc,
            {
                tokens: {
                    delimiter, description: desc, postDelimiter, start,
                },
            },
            idx,
        ) => {
...
export default iterateJsdoc(
    ({ context, jsdoc, utils }) => {
         ...
         * @returns The result of this check.
         */
        const isInvalidSingleLine = (tagName: string): boolean => (
            noSingleLineBlocks
                && (!tagName
                    || (!singleLineTags.includes(tagName)
                        && !singleLineTags.includes('*')))
        );
        ...
        if (
            checkForShortDescriptions(jsdoc, utils, requireSingleLineUnderCount!)
        ) {
            return;
        }
        ...
        if (noMultilineBlocks) {
            if (
                jsdoc.tags.length
                && (multilineTags.includes('*') || utils.hasATag(multilineTags as string[]))
            ) {
                lineChecks();
                        ...
                        source: [
                            {
                                ...
                                tokens: jsdoc.source.reduce(
                                    (
                                        ...
                                        if (typ) {
                                            Object.assign(obj, { type: typ });
                                        }

                                        if (tg && typ && nme) {
                                            Object.assign(obj, { postType });
                                        }

                                        if (nme) {
                                            Object.assign(obj, { name: obj.name + nme });
                                        }

                                        if (nme && desc) {
                                            Object.assign(obj, { postName });
                                        }

                                        Object.assign(obj, { description: obj.description + desc });
                                        ...
                                        && nameOrDescription.slice(-1) !== ' '
                                            ...
                                            Object.assign(obj, { description: `${obj.description} ` });
                                        }

                                        Object.assign(obj, { lineEnd });

                                        // Already filtered for multiple tags
                                        Object.assign(obj, { tag: obj.tag + tg });
                                        if (tg) {
                                            Object.assign(obj, { postTag: postTag || ' ' });
                                        }
