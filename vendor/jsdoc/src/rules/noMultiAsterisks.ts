...
export default iterateJsdoc(
    ({ context, jsdoc, utils }) => {
        ...
        jsdoc.source.some(({ number, tokens }) => {
            ...
            const singleLinePattern = allowWhitespace
                ? endAsterisksSingleLineNoBlockWS : endAsterisksSingleLineBlockWS;
            const multilinePattern = allowWhitespace
                ? endAsterisksMultipleLineNoBlockWS : endAsterisksMultipleLineBlockWS;
            const endAsterisks = isSingleLineBlock ? singleLinePattern : multilinePattern;
            ...
            const endFix = () => {
                if (!isSingleLineBlock) {
                    Object.assign(tokens, { delimiter: '' });
                }

                Object.assign(tokens, {
                    description: (description + delim).replace(
                        endAsterisks,
                        '',
                    ),
                });
